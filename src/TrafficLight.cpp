...
template <typename T> void MessageQueue<T>::send(T &&msg) {
  ...
  // std::lock_guard<std::mutex> as well as _condition.notify_one() to add
  // a new message to the queue and afterwards send a notification.
...
void TrafficLight::waitForGreen() {
  ...
  // I did not include infinite loop
...
void TrafficLight::simulate() {
  ...
  // in a thread when the public method „simulate“ is called.To do this, use the
  // thread queue in the base class.
...
void TrafficLight::cycleThroughPhases() {
  ...
  while (true) {
    time_between_loops = std::chrono::duration_cast<std::chrono::milliseconds>(
                             std::chrono::system_clock::now() - previous_update)
                             .count();
